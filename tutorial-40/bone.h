...
{
public:
    Bone(std::string name, int id, const aiNodeAnim *channel) : name_(std::move(name)), id_(id)
    {
    ...
    {
        auto translation = interpolate_position(time);
        auto rotation    = interpolate_rotation(time);
        auto scale       = interpolate_scale(time);
        transform_       = translation * rotation * scale;
    }

    int get_position_index(double time)
    {
    ...
    int get_rotation_index(double time)
    {
    ...
    int get_scale_index(double time)
    {
...
private:
    static double get_scale_factor(double last_ts, double next_ts, double time)
    {
        return (time - last_ts) / (next_ts - last_ts);
    }

    QMatrix4x4 interpolate_position(double time)
    {
        QMatrix4x4 position{};
        ...
        auto idx    = get_position_index(time);
        auto factor = get_scale_factor(positions_[idx].timestamp, positions_[idx + 1].timestamp, time);
        position.translate(positions_[idx].position * factor +
                           positions_[idx + 1].position * (1.0 - factor));
        return position;
    }

    QMatrix4x4 interpolate_rotation(double time)
    {
        QMatrix4x4 rm{};
        ...
        auto idx    = get_rotation_index(time);
        auto factor = get_scale_factor(rotations_[idx].timestamp, rotations_[idx + 1].timestamp, time);
        auto final_rotation =
    ...
    QMatrix4x4 interpolate_scale(double time)
    {
        QMatrix4x4 scale{};
        if (scales_.size() == 1) {
        ...
        auto idx    = get_scale_index(time);
        auto factor = get_scale_factor(scales_[idx].timestamp, scales_[idx + 1].timestamp, time);
